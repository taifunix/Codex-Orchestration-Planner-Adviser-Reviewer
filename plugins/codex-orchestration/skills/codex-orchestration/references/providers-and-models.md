...
| Claude Opus MCP route | Uses the same bounded compatibility launcher with a catalog-selected `claude-opus-*` model and Claude Code 2.1.219+ | Sealed cross-provider Planner or Advisor; at most one bundled Claude planning seat may be active. |
| Claude Sonnet/Opus MCP Reviewer route | Root-directed `review_code` invokes the authenticated Claude Code CLI with the exact persisted catalog-selected `claude-sonnet-*` or `claude-opus-*` model and no model-facing tools | Independent cross-provider Reviewer after Executor integration and verification; at most three total Reviewer reviews. |
